...
- Rechte: Planer/Admin/Abteilungsleiter via `_allPerms`, `role_architekt` r/w (`plaene`). Registriert: gema_auth (MODULES `plaene` cat Projektmanagement, FILE_MAP `pm_plaene`), index.html (PM, 19 Module), sw.js (v241), gema_recent, netlify.toml. KEIN GemaNotify-Event (org-intern). Boot-Guard: `plPrjSel` fehlt (Kein-Zugriff-Body) → Init bricht ab. Tests: Node-Engine 49 + Playwright-Smoke 45 (Seeding, Summen/EBF/Hülle exakt, Freigabe, Sanity-Ampel, KI-Apply gemockt inkl. Fenster, Override-Logik, Snap, CSV, Zugriff Admin/Architekt/Monteur, index-Kachel) + Fill-E2E 7 (synthetischer Grundriss auf Canvas: Closing schliesst 18-px-Tür, Fläche px-genau, Türleck → 60-%-Guard) + Vektor-E2E 8 (echtes pdf.js lokal, synthetisches Vektor-PDF mit Doppellinien-Wänden + Türblatt: Upload → Render → Segment-Extraktion < 1 px → Fill 51.5 m² exakt → Snap < 0.75 px).
