...
- **Objekt-AUSWAHL**: nie ein `<select>` mit allen Objekten (nach einer ERP-Migration sind es Tausende) — `GemaObjekte.comboHtml(feldId, objektId, opts)` + optional `comboBind(feldId, fn)` rendert ein Suchfeld; der gewählte Wert steht in einem versteckten Input mit `feldId` (bestehende `.value`-Leser bleiben gültig, `.options` nicht).
...
- Delta-Sync, IndexedDB-Warm-Cache und Hintergrund-Prefetch stecken zentral in gema_sync — Module brauchen dafür nichts zu tun. `loadCollection` paginiert (PostgREST deckelt bei 1000; Supabase-«Max Rows» nie unter 1000 senken); Seiten-Deckel 250 000 Rows, erreicht → Event `gema-sync-capped` + `lastCapped()` (Module melden es, nie still). Collections über 8 MB kommen beim Seitenstart NICHT mehr in den Speicher, sondern erst beim `bindCollection` der Seite (`ensureCached(key)` für reine Leser). **`setCached(key, VOLLER Pool)`** führt localStorage + Spiegel + IndexedDB in einem Zug nach — Massen-Schreiber und Module, deren eigenes `localStorage.setItem` an der Quota scheitern kann, nutzen es statt eigener Writes. `getCached` liefert IMMER Kopien (der Diff-Save vergleicht dagegen) — Hot-Paths memoisieren je Render (Muster `erpDocs`/`erpDocById` in pm_erp). POSTs haben ein Zeitlimit (60 s + 30 s/MB).
...
| Unternehmer | Ausschreibung, Offerten, Bestellungen, ERP r/w (FREMDER Unternehmer — kein Objekt/Termin/Stunden/Service) |
| `role_erp_sachbearbeiter` / `role_erp_abteilungsleiter` | Büro des eigenen Betriebs: ERP, Objekte, Termine, Stunden, Rapporte, Service, Bestellungen (Leitung zusätzlich Freigaben + Betriebsmittel). Ziel des Mitarbeiter-Imports. |
...
- **Paginierungs-Kanon (Drift-Guard `scripts/pdf_abschnitte_test.mjs`)**: Blöcke werden an Kindergrenzen geteilt, auch in Ein-Kind-Ketten (`.tab-content > .g-main-grid > Karten`) und im **Restplatz** eines Blatts (ab ~28 % frei); ein Block wird nur verkleinert, wenn er auch auf ein LEERES Blatt nicht passt (`GP.zwang`); mehrspaltige Karten-Raster werden untereinander gesetzt (`.gp-linear`). Beim Absteigen den Breiten-Fit lösen (`fitLoesen` — eine fixe Fit-Höhe macht sonst jedes Blatt «voll»); freier Platz NIE über `body.scrollHeight` (mit overflow:hidden nie < clientHeight), sondern `frei()` über die Unterkante der Kinder.
- **Klon-Regeln**: gemessene Scroll-Container (`data-gp-scroll`, overflow auto/scroll oder senkrecht gekappt) werden geöffnet — sonst fehlt alles, was am Bildschirm scrollte; `position:fixed` (Toasts) fliegt raus; **alle Mappen** (`.tab-content`) kommen mit Reiter-Titel (`.gp-mappe`) in den Bericht — Ausnahmen BENANNT: `data-gp-tabs="aktiv"` an der Reiterleiste (Varianten, sb_druckerhoehung) und `data-gp-print="weg"` an Referenz-Mappen; nackte Pfeil-Glyphen und leere Tabellenspalten (✕-Spalte) verschwinden; `th/td` dürfen umbrechen. Modul-Bedienelemente ohne Knopf-Markup (Sprung-Chips, Bedien-Hinweise) tragen `no-print`. `GemaSektion.hatWerte` kennt alle KPI-Muster (`.ww-kpi .v` …) — sonst gilt eine reine Ergebnis-Karte als leer.
...
- **`gema_qr_scanner.js`** (`GemaQR.scan/stop`, einziger QR-Dekodier-Pfad; `gema_nfc_scanner.js` fällt darauf zurück): **Reichweite hängt allein an den CSS-Pixeln des Suchers** — html5-qrcode zeichnet die qrbox in ein Canvas, das genau so viele Pixel hat wie die qrbox CSS-Pixel gross ist. Darum wird der Sucher intern mit `_basisBreite()` px aufgebaut und per CSS-`transform:scale` optisch verkleinert (`clientWidth` bleibt — die Bibliothek nutzt nirgends `getBoundingClientRect`); dazu Kamera-Auflösung im Sucher-Format, nativer BarcodeDetector, `disableFlip`, quadratische qrbox über die kurze Bildseite, Zoom-Regler + Licht nur bei gemeldeter Kamera-Fähigkeit (Zoom in `gema_qr_zoom_v1` gemerkt). Guard `scripts/qr_scan_reichweite_test.mjs` MISST Canvas-Grösse und Versuchsrate — nie eine fixe `qrbox` zurücksetzen.
...
| `sb_summenlinien` | Summenliniendiagramm eigenständig (aus Warmwasser Tab ⑤ gelöst). **Profile mitternachtsbasiert** (`SL_PROFILE`, Diagrammbeginn ist Parameter, Vorgabe 05:00); die 24 Stundenprozente sind Eingabefelder (Profilwahl füllt sie nur bei `isTrusted`, Herkunfts-Chip auto ⇄ eigene Eingabe, Σ-Normierung). Aufwärmzeit ⇄ Ladeleistung über `sl_lead` (V·4.187·∆θ), ∆θ aus KW/Speichertemperatur, 4 Ladefenster, Tagesablauf-Simulation. Tagesbedarf/Leistung/Speicher werden **einmalig** aus `wwSummary` übernommen (rein lesend). |
...
| `pm_erp` | Offerte→Auftrag→Rechnung, Positions-Editor (Rich-Text, Kalkulation, BKP-Titel, DataSelect, eigene Kataloge, Vorlagen), Swiss QR (Mod10, importierte ESR-Referenz gewinnt), Kreditoren mit Freigabe durch den Sachbearbeiter, Nachkalkulation, **Migration** aus dem Altsystem (`gema_erp_import.js`, 16 Abschnitte, idempotent, Reihenfolge in `IMPORT_REIHENFOLGE`): Zahlungsbedingungen · **Mitarbeitende** · Artikelstamm · Objekte · Adressen · **Bezugspersonen** · Offerten · Aufträge · Rechnungen · **Positionen** · **Zahlungen** · **Kreditoren** · **Anlagen** · **Termine** · **Stunden** · **Überträge**. **Mitarbeitende** werden wie `inviteBeteiligter` angelegt: `password:null` + Einladungs-Token, nie `role_admin` (Leitung→Abteilungsleiter, Sachbearbeiter→Unternehmer, sonst Monteur), Ausgetretene INAKTIV — `findeSachbearbeiter` findet auch Inaktive (Historie); Pensum nur gegen `wochenSoll+vorholProWocheH` plausibel, sonst Vermerk. **Lauf-Speicher**: während `ausfuehren` liegt jeder Pool einmal im Speicher (`laufStart`/`poolFlush`, Zwischen-Flush alle 500, Cloud via `saveRecords` in 200er-Blöcken, Lookups über `laufIndex`) — `poolSichern` je Datensatz wäre quadratisch. **Stunden-Writer**: Absenz-Zeile → `t.absenz={typ,stunden}` am Tag, NIE zusätzlich als Eintrag; Dedupe-Schlüssel `eintragKey` = Auftragsnr (ohne Tätigkeit — die Stufen beschriften verschieden), am Eintrag als `importKey`; `geaendert`-Flag entscheidet übers Speichern. Unbekannte Absenzarten werden als eigene GEMA-Absenzart angelegt (`absenzartenSicherstellen`, Regeln bleiben aus). Zuschlagspositionen (postyp 27) als `wert/modus`, nie `ep/menge`. Guards: `erp_stunden_writer_test` (fertiger Tagesrecord), `erp_pool_lauf_test`, `erp_absenzarten_test`, `erp_mitarbeiter_import_test`, `erp_belege_fixes_test`. Schreibt auch ausserhalb von `erp`: `einsatz:`/`std:`/`svanl:` mit dem jeweils richtigen moduleKey (`poolSichern(key,prefix,rec,mod)`). Positionen und Zahlungen schreiben GRUPPIERT je Beleg (ein Dokument trägt sein LV als Array); sie ersetzen nur die Sammelposition des Kopf-Imports (`importSammel`, `istSammelposition`) und rühren ein echtes LV nie an. Kalkulationswerte des Altsystems landen unter `importKalk` und NICHT in GEMAs Rechenfeldern — der importierte EP ist der fakturierte. Zahlbed-Kürzel sind Strings mit führender Null («00» = 60 Tage): nie über `parseInt`. Importierte Arbeitszeit trägt nur `dauerMin` statt von/bis — `stdEintragMin` zählt sie, `stdNachtMin` liefert ohne Uhrzeiten bewusst 0 (kein erfundener Nachtzuschlag). Die Zeiterfassung des Altsystems hat drei Stufen (Termin = Annahme → Handy = `erfasst` → Stundenmodul = `freigegeben`); `STUNDEN_RANG` entscheidet, welcher Wert gilt — eine Abweichung zwischen Stufe 2 und 3 ist die Korrektur, kein Konflikt. Ferien-/Überzeitüberträge (`arbueber`) sind SALDEN auf einen Stichtag, nie Summanden; sie liegen als `typ:'uebertrag'` im Stunden-Pool (Ferien in STUNDEN) — der Tagesrapport-Writer MUSS deshalb auf `!t.typ` filtern. Termin-Uhrzeiten kommen aus `von60`/`bis60` (Klartext), NIE aus `von`/`bis` (Dezimalzahlen); `parseZeit` deutet eine nackte Zahl bewusst nicht. Die Nachkalkulation des Altsystems wird als ein JSON-Feld nach `importNachkalk` mitgeführt und NIE eingerechnet (dort sind Lohn- und Gemeinkosten unparametriert). **`absenz` des Altsystems ist kein Absenzkatalog**: dieselbe Spalte führt Abwesenheiten, ARBEIT (Werkstatt/Büro/Sitzung) und Zuschlagsarten — `ABSENZ_MAP`/`absenzArt` trennt sie, Unbekanntes bekommt nie einen erfundenen Typ, `absenz.paid` ist als Kriterium unbrauchbar. Erkannte Absenzen werden zur GEMA-Absenz am TAG, nicht zum Arbeitseintrag. `hours.hrs_length` ist in SEKUNDEN (belegt: Ø 30 019/Tag = 8.34 h) — der Export teilt durch 3600, der Importer weist Zeilen über 24 h/Tag ab. `hrs_terminguid` trifft `termin.guid` (4 118 von 4 162; die frühere «0 Treffer»-Messung filterte auf `t.stunden>0`, und `termin.stunden` ist leer) — der Stunden-Writer zählt `terminVerknuepft`/`terminFehlt`, nicht auflösbare GUIDs bleiben als `importTerminId` am Eintrag, `norm()` vergleicht format-tolerant. `arbeiter` führt kein Pensum (`sollmo…sollfr`, `pctn`, `stdtime` leer) und `manager` ist leer — Teilzeit und Leitung werden in GEMA gesetzt, nie aus dem Export geraten. Details + Export-SQL in `KONZEPT_ERP_Migration_Altsystem.md`, Guards `scripts/erp_positionen_import_test.mjs` + `scripts/erp_uebertraege_import_test.mjs`. **Export-Paket `erp_export/`**: SQL GENERIERT aus den `<!-- export: NN_sektion -->`-Blöcken des Konzepts (`node scripts/erp_export_gen.mjs`), Runner `export.ps1` (mysql --batch, cmd-Umleitung, Jahresscheiben `{{JAHR}}`, `-PositionenAb`), Guard `erp_export_test` (Generat aktuell, JEDE Spalte hat ein Feld, Automatik erkennt die Sektion). Parser: `.tsv` = mysql-Modus (`NULL`→leer, `\n \t \\` zurück), Quote nur am Feldanfang (`Rohr 1/2"`), Grössen-Sperre 60/20 MB (`erp_parser_test`). Sektion aus dem Dateinamen (`erkenneSektion(headers,dateiname)`). Belege laufen über den Lauf-Speicher (`dokSuche`/`dokById`, Blöcke nach Anzahl UND Bytes), `poolFlush` → `GemaSync.setCached`, Cloud-Bilanz `report.cloud`; genau EIN Lauf (`laeuft()`), `abbrechen()`, `beforeunload`-Warnung; Berichte in `gema_erp_migration_berichte_v1`. Katalog max. 400 Artikel je Record (`importBasisName`), Benutzer in Tranchen ≤120 (Server-Deckel 200), Ausgetretene ohne Einladung, Arbeitsbereiche als `{id,name,farbe}`, Tagesrapport aus «freigegeben» = `genehmigt`, «bezahlt» ohne Datum → Zahlung auf Rechnungsdatum (beschriftet), MwSt 0 ist eine Aussage. `objekt1/2` des Altsystems ist eine ADRESSE (Strasse/Ort, am Auftrag «PLZ Ort») — belegt die KUNDENADRESSE (63.6 % wie der Zahler), keine Bezeichnung und keine zweite Objektadresse; sie bleibt Vermerk (`importObjekt1/2`) und füllt die Objektadresse NUR, wenn das Objekt keine hat (`importAdresseHerkunft`, `splitPlzOrt`; betrifft 2 Objekte). Konzept 8.16–8.18. `lkdir` (21 755 Ordner) ist nur der ORDNERNAME und endet auf die Alt-ID — er wandert in sieben Sektionen als `importOrdner` mit (Beleg, Kreditor, Objekt, Anlage, Adresse) und ist dort sichtbar; die Wurzel ist `…\OF_Zugeordnete_Dateien\<Kategorie>\<lkdir>` (aus `pdfforms.path`, Kategorie aus `lkmobiledir` → `importOrdnerKat`). **Der Adress-Schlüssel ist `adressen.id`, NIE `oknummer`** (int, trifft `obj.knummer` 4 381/4 417; `oknummer` ist fast überall NULL) — sonst zeigen die drei Adress-Slots am Objekt und der Offert-Kunde ins Leere und jede Adresse entsteht doppelt (Guards `erp_export_test` 4b + `erp_import_betrieb_test` 15). Anlagen ohne künftige Revision kommen **inaktiv** (`importInaktivGrund`, `opts.anlagenAltInaktiv`) — sonst legt sv_service beim ersten Öffnen ~180 Serviceaufträge an; Stichtag «als bezahlt» auch im Auto-Import; Spesen liegen zusätzlich am Eintrag (`importSpesen`). Guard `erp_import_betrieb_test`. pm_erp: Listen-Deckel 300 + «Mehr laden» (`erpMehrHtml`), Kunden-Suchfeld (`erpKundeComboHtml`), Zahlbed-Editor bewahrt `tage:null`/Import-Felder, Nachrender nach dem Bind, Vermerke (Nachkalkulation `_ERP_NK_LBL`, Position `erpPosAltInfo`, Kreditor `erpKredVermerkeHtml`), Kunden-Rabatt wird übernommen (`rabattHerkunft`), Skonto ist Kondition und wird NIE abgezogen (`erpSkonto`). Positionsspalten konfigurierbar (`ERP_POSCOLS`, `erpPosColsPdf` filtert Editor-Chrome). WYSIWYG-Umbrüche: Bezeichnungsspalte exakt 94 mm + `box-sizing:border-box`. Auto-Save mit Status-Indikator; `_acked`-Signatur-Diff. |
...
| `pm_stunden` | GAV-Zeiterfassung (Zuschläge, Töpfe A/B, Vorholzeit, Absenzen mit Regeln, Ferienanträge, Betriebsferien, Pensum-Skalierung). **Freigabe pro TAG** mit drei Gates: Material-Frage, Foto-Pflicht, Abschluss-Rückmeldung. Geplante Termine werden ohne Klick mit Planzeit übernommen; «nicht stattgefunden» ist von der Zeiterfassung unabhängig. **Ferien-/Überzeitübertrag** (`typ:'uebertrag'` im selben Pool, `stUebertrag(userId,jahr)` → `opts.uebertrag`): sein Guthaben ENTHÄLT den Anspruch der Periode bereits — NIE zusätzlich `stdFerienAnspruch` addieren. Stunden→Tage über `wochenSoll/5` (mit Pensum, ohne Vorholzeit — `stdTagSollH` wäre falsch). Nur der Stichtag DESSELBEN Jahres gilt; Ferien und Überzeit haben je ein eigenes Fenster ab Stichtag (nur wenn der Übertrag den Wert führt). Ist/Soll der Jahresbilanz sind TAGESGENAU über Werktage in Wochen mit Rapporten (die Wochenrechnung zählte Randwochen um Neujahr voll: perfektes Jahr = −32 h); Töpfe A/B bleiben wochenweise. Fortschreibung ins neue Jahr über `stUebertragErfassen`. Importierte Einträge tragen nur `dauerMin` — pm_erp, pm_einsatzplan und sys_workspace lesen es ebenfalls; der Eintrags-Editor hat dafür einen Dauer-Modus (`stEinModus`) und MERGT beim Speichern über den alten Eintrag (Import-Marker bleiben, sonst Dublette beim nächsten Lauf). `stdAbsenzAnteil(t,T)` rechnet stundenbasierte Absenzen anteilig. Guard `scripts/stunden_uebertrag_test.mjs`. |
...
| `pm_abnahme` | SIA 118, mehrere Protokolle pro Objekt (`abproto:` mit `scopeKey`), Checkliste Installationswände, Teilnehmer/Freigaben cross-org, Monteur-Mängellisten (auch extern per E-Mail; Abarbeiter sieht NUR Mängel + Projektangaben). Scope-Regel: `_abScopeKey()` bei jedem Wechsel neu, Protokoll trägt `_scope`. PDF im Brand-Stil, Unterschriften auf Seite 1. **Fortschritt der übergebenen Mängel ist live sichtbar**: `abMlIdx()` (memoisiert, `abMlIdxInvalidate()` bei jedem Pool-Write) speist Badge + Info-Block an der Mangel-Karte und das Panel «Übergeben — in Arbeit» (Filter `abMlDarfKontrollieren`). **NUR LESEND** — `it.erledigt` schreibt allein `abMlFreigeben`, und das übernimmt nur Punkte mit `status==='erledigt'`. **Mängelstand** rechnet `abMaengelStand()` (Protokoll + Live-Stand der Listen) — EINE Quelle für den Block `#maengelStand` (Bildschirm), die Druck-Kopie `#maengelStandPrint` in `#view_abnahme` (`.stand-print`, nur `@media print` — gedruckt wird die sichtbare Ansicht) und das PDF (Deckblatt-Zeile, Fortschritts-Kachel «FERTIG …» in Grün, Spalte «Status / Erledigt» statt leerer Zelle; Spaltensumme bleibt 515 pt). **Beweisfotos** stehen im PDF in einem eigenen Abschnitt «Beweisfotos zur Mängelbehebung» — Vorher/Nachher nebeneinander (250 × 150 pt) mit Monteur, Datum und Kommentar; Quelle sind die laufende Liste UND die bei der Freigabe als `beweis:true` markierten Kopien, der Foto-Anhang zeigt nur noch Mangel-Aufnahmen. Der Nachweis-Block hängt an der Mangel-Karte (nicht im Detail), damit er im Ausdruck auch zugeklappt erscheint (Fotos dort 120 px). Sind ALLE Punkte erledigt, meldet `_abMaengelFertigPruefen()` das EINMAL an die verantwortlichen Seiten der übergebenen Listen (Stempel `state.maengelFertigAm`, nie an die auslösende Person; ein wieder geöffneter Punkt löscht den Stempel). Der Abarbeiter setzt je Punkt **drei Status-Knöpfe rechts** (`abMlSegHtml` → `abMlItemStatus`: `offen` | `in_arbeit` | `erledigt`; «Offen» ist der Weg zurück, `abMlItemToggle` bleibt als Wrapper). Fällt der letzte Haken, meldet das EINMAL (Stempel `alleErledigtAm`); `kommentarVerantwortlicher` einer Zurückweisung bleibt als Begründung stehen. `abPoolSave` schreibt über `GemaSync.setCached` (eigenes `localStorage.setItem` verlor bei Quota-Fehler den ganzen Record). **Gegenbestätigung des Planers**: `abMlAkzeptieren(mlId,itemId)` / `abMlAkzeptierenAuswahl` (Mehrfachauswahl `_abMlSel`) / `abMlAkzeptierenAlle` setzen `status='akzeptiert'` — nur auf Punkten, die der Abarbeiter als `erledigt` gemeldet hat; erst die Bestätigung schreibt Visum und Beweisfoto-Kopien ins Protokoll (`_abMlPunktInsProtokoll`). `abMlRueckmeldung` nimmt beides punktgenau wieder zurück (`_abMlPunktAusProtokoll`, `beweisMlItem`-Marke), hängt die Begründung an den Punkt und setzt ihn **automatisch auf `in_arbeit`** — der Abarbeiter sieht sie dort als Rückmeldung, ein `akzeptiert`-Punkt ist für ihn gesperrt. Sind alle Punkte bestätigt, geht die Liste auf `freigegeben`; `_abMlFertig` = `erledigt`|`akzeptiert`. **Beweisfotos sind einzeln verwerfbar** (`abMlFotoLoeschen`, ✕ am Daumennagel, im Ausdruck ausgeblendet): der Abarbeiter solange sein Punkt nicht `akzeptiert` ist, die verantwortliche Seite immer — die Rechteprüfung läuft VOR und NACH dem Dialog (`_abMlFotoDarf`). Eine schon übernommene Kopie geht über `beweisMlItem` + Quelle exakt mit; Mangel-Aufnahmen des Planers tragen dort kein ✕ (die pflegt er im Protokoll, `deletePhoto` → GemaDialog). **Im PDF gewinnt die Protokoll-Kopie**: gibt es `beweis`-Fotos am Punkt, liefert die laufende Liste nichts mehr dazu — sonst stand jedes bestätigte Bild doppelt und ein im Protokoll gelöschtes kam über die Liste zurück. Guard `scripts/abnahme_maengel_fortschritt_test.mjs`. |
...
| Modul | Kern |
|---|---|
| `if_werkzeug` | Multi-Tenant-Pool (orgId-Filter, fremde Orgs beim Save erhalten). Koffer (`kofferInhalt`, Kontrolle/Rückgabe, Bündelung in der Liste), QR/NFC (write-on-detect!), Etiketten 49×23 mm (1-Bit-Monochrom fürs Thermodruck), Sammelerfassung/-bearbeitung/-ausleihe, Prüfnachweis Elektro (`gema_pruefwerte.js` geteilt mit dem Lieferanten-Dashboard), Audit-PDF, CSV-Vollexport, Direkteinbuchung durch Lieferanten, Prüfmodus «liegt das Richtige am richtigen Ort». **Lieferanten-Feld**: Vorschläge in der Reihenfolge registrierte GEMA-Lieferanten (schon verwendet → weitere, `_wzRegistrierteLieferanten` aus dem `lieferant:`-Pool + Konten) → eigene Freitexte; nur registrierte tragen `supplierId` (Konto) + `supplierLieferantId` (Firma), manuelles Tippen löst beide. **Defektmeldung** geht auf Wunsch direkt ans ganze Lieferanten-Team (`_wzSendDefektAnLieferant`, Haken im Melde-Dialog) — ohne zustellbares Konto wird das benannt; sie bleibt bearbeitbar, löschbar und «erledigt» rücknehmbar (`_wzCanEditBericht` = Magaziner ODER Melder, nur `typ==='defekt'` — Prüfberichte sind Nachweis). Test-Hook `window._wzHooks`. Guard `scripts/werkzeug_lieferant_defekt_test.mjs`. |
...
- `nav_uniform_test` (Nav-Kanon) · `workspace_module_test` (Modul-Registrierung) · `summenlinien_test` (Diagramm-Modul, Gegenprobe gegen die Warmwasser-Engine) · `notify_prefs_gating_test` (Notify-Gruppen) · `verknuepfung_test` (Werte-Katalog aktuell) · `elektro_basis_test` (el_-Baukasten) · `pdf_opsz_test` · `mobile_kompakt_test` · `datumsfelder_test` · `scroll_stabilitaet_test` · `sync_outbox_test` / `sync_offline_meldung_test` / `sync_tokenless_guard_test` / `loading_perf_test` · `kette_e2e_test` · `kein_default_admin_test` · `storage_delete_test` · `fotoqueue_offline_test` · ERP-Migration: `erp_export_test` · `erp_parser_test` · `erp_import_betrieb_test` · `erp_stunden_writer_test` (+ die übrigen `erp_*_import_test`).
- `werkzeug_lieferant_defekt_test` — Lieferanten-Vorschläge + Zustellung/Korrektur der Defektmeldung, Schadensbericht-Erfassung bearbeitbar, Handy-Ansicht der Erfassungs-Phase.
- `abnahme_maengel_fortschritt_test` — zwei Browser-Kontexte (Planer/fremder Monteur) an EINER gemockten Cloud: Teilfortschritt sichtbar, kein stiller Rückschrieb, Quota-Fehler verliert kein Häkchen, leerer Cloud-Pull leert kein Protokoll.
