...
| `pm_erp` | Offerte→Auftrag→Rechnung, Positions-Editor (Rich-Text, Kalkulation, BKP-Titel, DataSelect, eigene Kataloge, Vorlagen), Swiss QR (Mod10, importierte ESR-Referenz gewinnt), Kreditoren mit Freigabe durch den Sachbearbeiter, Nachkalkulation, **Migration** aus dem Altsystem (`gema_erp_import.js`, 14 Abschnitte, idempotent, Reihenfolge in `IMPORT_REIHENFOLGE`): Zahlungsbedingungen · Artikelstamm · Objekte · Adressen · **Bezugspersonen** · Offerten · Aufträge · Rechnungen · **Positionen** · **Zahlungen** · **Kreditoren** · **Anlagen** · **Termine** · **Stunden**. Schreibt auch ausserhalb von `erp`: `einsatz:`/`std:`/`svanl:` mit dem jeweils richtigen moduleKey (`poolSichern(key,prefix,rec,mod)`). Positionen und Zahlungen schreiben GRUPPIERT je Beleg (ein Dokument trägt sein LV als Array); sie ersetzen nur die Sammelposition des Kopf-Imports (`importSammel`, `istSammelposition`) und rühren ein echtes LV nie an. Kalkulationswerte des Altsystems landen unter `importKalk` und NICHT in GEMAs Rechenfeldern — der importierte EP ist der fakturierte. Zahlbed-Kürzel sind Strings mit führender Null («00» = 60 Tage): nie über `parseInt`. Importierte Arbeitszeit trägt nur `dauerMin` statt von/bis — `stdEintragMin` zählt sie, `stdNachtMin` liefert ohne Uhrzeiten bewusst 0 (kein erfundener Nachtzuschlag). Die Zeiterfassung des Altsystems hat drei Stufen (Termin = Annahme → Handy = `erfasst` → Stundenmodul = `freigegeben`); `STUNDEN_RANG` entscheidet, welcher Wert gilt — eine Abweichung zwischen Stufe 2 und 3 ist die Korrektur, kein Konflikt. Details + Export-SQL in `KONZEPT_ERP_Migration_Altsystem.md`, Guard `scripts/erp_positionen_import_test.mjs`. Positionsspalten konfigurierbar (`ERP_POSCOLS`, `erpPosColsPdf` filtert Editor-Chrome). WYSIWYG-Umbrüche: Bezeichnungsspalte exakt 94 mm + `box-sizing:border-box`. Auto-Save mit Status-Indikator; `_acked`-Signatur-Diff. |
