...
- **`format` = Ziel-Export-Format (KRITISCH — «kein JSON»/langsam-Fix, 07/2026)**: Der DataExpert-Parameter `format` wählt das **Ziel-System-Exportformat**, NICHT JSON-vs-XML als Transport. `debim` (DataExpert®BIM) ist schwer (BIM-Attribute, Bilder inline, liefert **XML/Nicht-JSON**) → langsam + `JSON.parse`-Fehler. **Default ist darum `bexio`** (leichtgewichtiges Swiss-ERP-JSON: Artikel-Nr, Kurzbeschreibung, Preis, Einheit, EAN — keine BIM-Attribute/Bilder). Pro Modus konfigurierbar: `DATASELECT_FORMAT_SUCHE` (Suche) / `DATASELECT_FORMAT_BILD` (Detail-Abruf beim Einfügen), Default beide `bexio`. Der Proxy liest die Antwort **robust** (BOM strippen, JSON-Parse; bei HTML/XML `<`-Anfang → Meldung «gewähltes Format liefert XML statt JSON, bitte JSON-Format wie bexio setzen»).
- **Bilder verzögert laden (Performance, 07/2026)**: Die **Suche liefert KEINE Bilder** (schnelle, schlanke Trefferliste — bexio trägt ohnehin keine) und markiert pro Artikel nur `hatBild`. Der Proxy entfernt im Suchmodus schwere Inline-Bilder (`data:`-URIs/Base64) aus dem Payload; HTTP-URLs bleiben (billige Strings, das Frontend zeigt sie in der Suche NICHT). **Erst beim Einfügen einer Position** wird das Bild des GEWÄHLTEN Artikels nachgeladen (Detail-Abruf `bilder=1`+`format=DATASELECT_FORMAT_BILD` nur falls die Suche keine URL hatte), im Browser auf **~340 px verkleinert + JPEG-komprimiert** (`erpDsShrink`, Qualität 0.72) → wenige Bytes, Anzeige ~4×4 cm; ohne CORS (getaintetes Canvas) Fallback auf die Roh-URL. Der Insert bleibt sofort, das Bild «poppt» asynchron nach (`erpDsLoadImage`).
- **Schema-Robustheit (KRITISCH)**: Das exakte Feldschema ist format-/vertrags-/versionsabhängig. `_normArtikel`/`_extractArray` bilden darum **heuristisch** über viele plausible Feldnamen (bexio: `intern_code/code`, `intern_name/intern_description`, `sale_price/default_price/purchase_price`, `unit_name`; debim/generisch: ArtNr/artikelnr/articleNumber, Bezeichnung/Bez(+Bez2)/description, Bruttopreis/listPrice, Einheit/VPE/unit, Bild/Bilder[]/images[{url}]/data:-URL, Container `{Artikel:[…]}`/`{data:[…]}`/Array/Einzelobjekt) auf `{artnr,bezeichnung,ean,preis,waehrung,einheit,hersteller,serie,bildUrl}` ab; Schweizer/deutsche Preisformate (`1'234.50`/`1.234,50`/`12.30 CHF`) via `_num`. Weicht eine reale Antwort ab → Kandidatenlisten in beiden Dateien ergänzen (Node-Test deckt die Varianten ab). Exporte `_normArtikel/_extractArray/_num/_pick/_bild` fürs Node-Testing.
...
- **Diagnose «🔍 Rohantwort prüfen» (07/2026, `scripts/dataselect_debug_test.mjs` 10 Checks)**: Pro IGH-Lieferant in der Sidebar ein Link, der die **rohe** Antwort von dataselect.ch abruft und im Dialog zeigt — HTTP-Status, Content-Type, **erkanntes Format** (JSON/XML/HTML/leer), JSON-parsebar, Länge, **erste ~2500 Zeichen** + die abgefragte URL (Token redigiert) — mit Deutung (403 = Vertrag nötig · XML statt JSON → `DATASELECT_FORMAT_SUCHE=bexio` · JSON ✓ · Netzwerkfehler). So sieht man ohne IGH-Wissen, WAS ein Lieferant zurückgibt. Proxy: `?debug=1` (JWT-gated, Key nie im `triedUrl`), optional `&format=<x>` zum Testen anderer Zielformate; Client `GemaDataSelect.debug({anbieter,artnr?|bez?,format?})`; UI `erpDsDebug`/`erpDsDebugFormat` (Dialog via `GemaDialog.alert({html:true})` — neuer opt-in `html`-Flag im geteilten Dialog, Aufrufer escapt selbst).
- Registriert: sw.js (CACHE_FILES), Script-Include in pm_erp, netlify.toml (`/api/dataselect`). ENV (Netlify, alle optional): `DATASELECT_BASE`, `DATASELECT_API_KEY`, `DATASELECT_KEY_PARAM`, `DATASELECT_FORMAT_SUCHE` (Default `bexio`), `DATASELECT_FORMAT_BILD` (Default `bexio`). Ohne Deploy/Key läuft das Modul weiter (Picker meldet den Fehler, restliche Positions-Quellen unberührt).
