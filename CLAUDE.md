...
- **Besonderheiten + Schlüssel/Zutritt (07/2026, `scripts/einsatzplan_besonderheiten_test.mjs` 23 Checks)**: `org.settings.einsatzplan.besonderheiten = [{id:'bs_<slug>', ic, label}]` — frei definierbare Monteur-Hinweise («📦 Material im Lager bereit», «🪜 Grosse Leiter mitnehmen» …), Editor im ⚙️ (Icon+Label-Zeilen, «↺ Beispiele einfügen» = `BS_VORLAGEN`, IDs bleiben beim Umbenennen stabil via `data-id`). Pro Termin Mehrfachauswahl (`ev.besonderheiten=[ids]`, Toggle-Chips im Modal) **PLUS Freitext DIREKT im Termin** (Feedback 23.07., `scripts/stunden_freigabe_foto_test.mjs`): `ev.besonderheitenFrei=['Schuhe ausziehen beim Kunden',…]` — Eingabefeld im Termin-Modal (`epBesFreiAdd`/`epBesFreiDel`, Sektion `epBesSection` immer sichtbar für Planer), **☆ = «als Vorlage speichern»** (`epBesFreiVorlage`: nimmt den Text in die org-weite ⚙️-Liste auf und hängt den Termin auf den Vorlagen-Chip um; best effort), im nativen Termin-Sheet als `;`-getrenntes Feld `data-f="besfrei"`. **Alle Anzeigen laufen über `bsAlle(ev)`** (= Vorlagen-Chips + Freitext mit 📌-Icon) — gut sichtbar in allen Sichten: Icons mit Tooltip in Woche/Monat (`bsIconsHtml`/`bsIconsText`), volle amber Pills in Tages-Modal + «Meine Woche» (`bsChipsHtml`) und in der pm_stunden-Karte «Geplante Termine» (`stBesChips` liest dieselbe Settings-Liste + merged Freitext); Notify-Text nennt die Besonderheiten. Dazu trägt der **ERP-Auftrag `schluessel:{code,info}`** (🔑-Sektion «Schlüssel / Zutritt» in den Auftrag-Grunddaten von pm_erp — Schlüsselcode/Tresor-Code + «wo/bei wem abholen»; nur typ auftrag): der Einsatz zeigt Code + Abholort **live vom Auftrag** (`epSchluessel` via ERP-Pool-Lookup mit Org-Guard + 1.5s-Memo `_epErpMemo`; Pool wird beim Boot für ALLE gebunden — auch Monteure) als 🔑-Box in «Meine Woche», Tages-Modal und im Einsatz-Modal (`#evKeyInfo`), Woche/Monat markieren mit 🔑. **Der Schlüsselcode geht NIE in Notify-Records** (nur Besonderheiten-Labels).
