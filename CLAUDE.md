...
- **Auftrag-Einsatz (Feedback 19.07.2026, `scripts/erp_einsatz_auftrag_test.mjs`)**: Neuer Einsatz startet als **Typ «Auftrag»** (`epNeu` Default `typ:'auftrag'`). Bei Typ Auftrag ist die separate Projekt/Objekt-Auswahl **ausgeblendet** (`evObjektWrap` display:none — das Objekt kommt aus dem Auftrag). Die Auftrag-Auswahl ist eine **Such-Maske** (Modal `#aufModal`, kein Dropdown mehr): `epAufOpen`→`epAufRender(q)` listet die offenen Aufträge **sortiert nach `erstelltAm` (neueste zuerst)**, Filter über Nr/Titel/Kunde/Strasse/PLZ/Ort/objektName; `epAufPick(id)` übernimmt Auftrag + verknüpftes Objekt/Bereich (getippter Titel bleibt via `epEvCollect`), `epAufPickRender` zeigt den gewählten Auftrag als Chip mit «Ändern». `epAuftragById` löst live aus dem ERP-Pool auf; der alte `ev_auftrag`-Select + `epEvAuftragChanged` sind entfernt (`epEvSave` liest `curEv.auftragId`).
