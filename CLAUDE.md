...
- Test-Hooks: `window._weHooks` (`settings/liefStatus/posStatus/extractHtmlGrid/extractTextGrid/guessCols/applyGridMapping/pdfItemsToGrid/applyPdfBands/mkPos/projModus/gemaObjekteAn/freieAdrAn/defaultProjekt/lieferantKey/findMapping/saveMapping`). Playwright-Smoke (localStorage-Seeding Lagerist vs. Monteur, externe Hosts geblockt): Zugriff/4 Tabs/Etiketten-Auto-Fit/HTML-+Text-Grid-Extraktion+Spalten-Heuristik/«Kein Zugriff» für Monteur; plus Projektmodus-Suite (beides/gema/frei: Picker-Sichten, freies Adressformular, Namens-gekeyte Mappings case-insensitive, Schnell-Etikette ohne GEMA-Dropdown im frei-Modus).
