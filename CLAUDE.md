...
Pro Objekt beliebige Behörden-/Amtsformulare (PDF) ausfüllen: die KI erkennt die Felder, Objekt-/Beteiligtendaten werden vorbefüllt (überall manuell überschreibbar), ausfüllbare PDFs werden direkt als offizielles Dokument exportiert, und ein Watcher überwacht die Quell-URL auf Änderungen. Kategorie Projektmanagement.

- **Pools (moduleKey `behoerden_formulare`)**: Formular-Instanzen `bform:`→`gema_bform_pool_v1` (org-intern, Einträge tragen nur die Storage-URL des Blanko-PDF, Base64 nur Fallback ≤ 2.5 MB) · Büro-Vorlagen `bformv:`→`gema_bform_vorl_pool_v1` (Feld-Schema wiederverwendbar). Event `gema-bform-changed`; Boot Stale-while-revalidate (`_cloudLoaded`). Einzel-Saves via `GemaSync.saveRecord`.
- **Engine im `/*ENGINE-START*/…/*ENGINE-END*/`-Block** (Node-testbar, 28 Fälle grün): `BF_SOURCES` (gemaMap-Katalog gruppiert Objekt/Bauherr/Architekt/Planer/Unternehmer/Eigene Firma/Sonstiges) + `BF_ROLLE_MAP` (gemaMap-Präfix → Beteiligten-Rolle); `bfResolveMap(key,ctx)` löst einen gemaMap-Schlüssel gegen `{objekt,beteiligte,org,user,heute}` auf (Beteiligte via Rollen-Match, org.telefon/email aus `org.settings.erp`); `bfAutoFill(felder,ctx)` (nur nicht-manuelle Felder, überschreibt nie mit leerem Resolve); `bfMergeFields(acroFields,aiFelder)` (AcroForm-Feldname+Typ+Options sind autoritativ, Label+gemaMap von der KI per Namensmatch; flaches PDF = nur KI-Felder); `bfVollstaendigkeit`, `bfMapLabel`.
...
- **Ausfüllen/Export**: AcroForm-PDFs werden mit **pdf-lib** direkt befüllt (`bfFillPdf`: `getTextField/getCheckBox/getDropdown/getRadioGroup`, `updateFieldAppearances`, Blob-Download) → offizielles Behörden-PDF. Immer verfügbar: «📄 Datenblatt» (gebrandetes Print-Fenster mit Label/Wert-Tabelle) als Fallback für flache/gescannte PDFs. Original-PDF-Bytes via `_origBytes` (Storage-URL mit `/sb`-Proxy-Fallback bzw. dataUrl).
- **Feld-UI**: pro Feld Label + gemaMap-Select (gruppiert) + Wert-Input (text/checkbox/dropdown mit Options); Werteingabe setzt `manuell:true`; gemaMap-Wechsel löst neu auf und löscht `manuell`. «↻ Aus Projekt neu befüllen» wendet alle gemaMaps auf nicht-manuelle Felder an. «＋ Feld» für manuelle Ergänzungen.
- **Watcher**: `sourceUrl` pro Formular. `netlify/functions/form-watch.js` (GET `?url=` → SHA-256-Hash + ETag/Last-Modified/Size, server-seitig gegen CORS, SSRF-Guard blockt interne Hosts). Client «🔎 Jetzt prüfen»/«🔎 Alle prüfen»: setzt beim ersten Mal die Baseline (`sourceHash`), danach `sourceChanged=true` bei Abweichung + Notify `behoerde_formular_geaendert` an `role_admin`. **Automatik**: geplante Function `form-watch-cron.js` (`[functions."form-watch-cron"] schedule="@daily"` in netlify.toml, Service-Key) prüft täglich ALLE `bform:`-URLs server-seitig, markiert geänderte Records und schreibt die Admin-Notifikation direkt als `notif:`-Row (auch ohne dass jemand das Modul öffnet). Ohne Service-Key = No-Op.
- Rechte: `GemaAuth.can('write','behoerden_formulare')` — Planer/Admin/Abteilungsleiter via `_allPerms`, `role_architekt` r/w (Baugesuche). Registriert: gema_auth (MODULES `behoerden_formulare`, FILE_MAP `pm_behoerden_formulare`), gema_notify (`behoerde_formular_geaendert`), index.html (PM, 18 Module), sw.js (v234), gema_recent, netlify.toml (Redirects `/api/claude-formfields`, `/api/form-watch` + Cron-Schedule). ENV: `ANTHROPIC_API_KEY` (KI), `SUPABASE_SERVICE_KEY` (Cron). Test-Hooks `window._bfHooks`. Nutzt die neue CDN-Dependency pdf-lib 1.17.1 (lazy, wie Revisionsunterlagen).
