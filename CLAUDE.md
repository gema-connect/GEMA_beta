...
| `pm_erp` | Offerte→Auftrag→Rechnung, Positions-Editor (Rich-Text, Kalkulation, BKP-Titel, DataSelect, eigene Kataloge, Vorlagen), Swiss QR (Mod10, importierte ESR-Referenz gewinnt), Kreditoren mit Freigabe durch den Sachbearbeiter, Nachkalkulation, **Migration** aus dem Altsystem (`gema_erp_import.js`, 10 Abschnitte, idempotent, Reihenfolge in `IMPORT_REIHENFOLGE`): Zahlungsbedingungen · Artikelstamm · Objekte · Adressen · Offerten · Aufträge · Rechnungen · **Positionen** · **Zahlungen** · **Kreditoren**. Positionen und Zahlungen schreiben GRUPPIERT je Beleg (ein Dokument trägt sein LV als Array); sie ersetzen nur die Sammelposition des Kopf-Imports (`importSammel`, `istSammelposition`) und rühren ein echtes LV nie an. Kalkulationswerte des Altsystems landen unter `importKalk` und NICHT in GEMAs Rechenfeldern — der importierte EP ist der fakturierte. Zahlbed-Kürzel sind Strings mit führender Null («00» = 60 Tage): nie über `parseInt`. Details + Export-SQL in `KONZEPT_ERP_Migration_Altsystem.md`, Guard `scripts/erp_positionen_import_test.mjs`. Positionsspalten konfigurierbar (`ERP_POSCOLS`, `erpPosColsPdf` filtert Editor-Chrome). WYSIWYG-Umbrüche: Bezeichnungsspalte exakt 94 mm + `box-sizing:border-box`. Auto-Save mit Status-Indikator; `_acked`-Signatur-Diff. |
