...
- Registriert: sw.js v342 (gema-native.css/js + gema_native_mobil.js), `<link gema-native.css>` in den 6 Modul-Heads; die Einstellung liegt in `sys_profil.html`. Test: `scripts/native_screens_smoke_test.mjs` (59 Checks — Phone-Viewport 390×844 mit PostgREST-Row-Mock, alle 6 Screens mit echten Daten, für pm_stunden/pm_einsatzplan der komplette native Erfass-Roundtrip [Sheet öffnen → Felder → Speichern → echter Pool-Eintrag → Bearbeiten-Sheet], Einstellungs-Toggle klassisch/native statt In-Modul-Umschalter, Desktop-Gegenprobe).
