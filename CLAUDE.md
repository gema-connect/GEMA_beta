...
**Feedback-Umbau 07/2026 (Sandro, `scripts/abnahme_feedback_test.mjs` 58 Checks — deckt auch Auto-Save/Migration/Adoption und die Apparateliste ab):**
...
- **«Durch wen»-Vorschlag** = Unternehmer aus dem Protokollkopf (`kuerzelVorschlag()`, erster Adressteil, überschreibbar) bei neuen Punkten + als Platzhalter.
- **Unterschriften**: Labels «Name / Vorname» + **«Firma» statt Visum** (Storage-Key bleibt `sig.<k>.visum`) — Firma wird aus dem Kopf vorbefüllt (Unternehmer/Bauherr/Bauleitung; bewusst geleerte Felder via `_visumTouched` nie erneut befüllt); gezeichnete Unterschriften speichern **automatisch** (kein eigener Button — siehe Auto-Speichern).
...
- **Arbeitsgattung mit komplettem BKP-Katalog** als Datalist (`gema_bkp_katalog.js` eingebunden, 349 Einträge «BKP <Nr> <Titel>»).
