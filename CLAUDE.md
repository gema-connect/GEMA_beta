...
- **pm_stunden (voll nativ)**: Wochen-Summe + Tages-Gruppen; ＋ Zeit erfassen / Eintrag-Tap öffnen **native Sheets** (Von/Bis/Pause/Projekt/Tätigkeit + Einsatz-Übernahme) → Kette `stEinNeu`/`stEinEdit(datum,id)` → klassische `ein_*`-Felder befüllen → `stEinSave()`; Löschen über `stEinDelete()`. Absenz-Sheet (Typ/Umfang) → `stAbsOpen`+`stAbsSave`/`stAbsRemove`. «Woche einreichen» → `stWocheEinreichen`. Refresh: `window.stRender` gewrappt.
