...
**iPhone-Ansicht in den Modulen (gema_native_mobil.js, 07/2026 — die 6 vorbereiteten Screens sind LIVE verdrahtet):** `GemaNativeMobil.mount({render})` hängt einen `.gn.gn--page`-Vollbild-Screen (fixed, z-index 900) in die Modulseite: aktiv NUR ≤740px (`phone()`), Desktop/Tablet unverändert; `html.gn-native-on` versteckt `.g-nav` + sperrt Body-Scroll; Umschalter **«Klassische Ansicht»** (`[data-gn-classic]`-Button im Screen, persistiert in `gema_native_view_v1`, URL-Override `?native=0/1`) + **📱-Rückkehr-Pill** unten links im klassischen Phone-Modus; Re-Render erhält die Scroll-Position; `esc()` = Voll-Escaper. **z-index-KRITISCH:** viele Modul-Modals (`.modal-bg`) liegen bei z-index 900 = gleich wie `.gn--page` → der Screen läge davor und würde sie verdecken. `html.gn-native-on .modal-bg{z-index:10600!important}` hebt klassische Modals im Native-Modus über den Screen (Sicherheitsnetz für selten genutzte Sub-Dialoge: Auftrag-Suche, Einstellungen). Verdrahtet in 6 Modulen (Muster: Integrations-Block am Dateiende, ECHTE Daten + bestehende Modul-Aktionen):
...
- Registriert: sw.js v341 (gema-native.css/js + gema_native_mobil.js), `<link gema-native.css>` in den 6 Modul-Heads. Test: `scripts/native_screens_smoke_test.mjs` (58 Checks — Phone-Viewport 390×844 mit PostgREST-Row-Mock, alle 6 Screens mit echten Daten, für pm_stunden/pm_einsatzplan der komplette native Erfass-Roundtrip [Sheet öffnen → Felder → Speichern → echter Pool-Eintrag → Bearbeiten-Sheet], Desktop-Gegenprobe).
