...
- Rechte: Planer/Admin/AL via `_allPerms`; `role_architekt`/`role_unternehmer` r/w; `role_monteur`/`role_spengler`/`role_bauherrschaft` read (+ Pendenzen erledigen). Registriert: gema_auth (MODULES `planablage`, FILE_MAP `pm_planablage`), gema_notify (3 `plan_*`-Keys), gema_notify_ui (MODUL_LABELS «📐 Planablage & Pendenzen» + MODUL_ZUGRIFF), index.html (PM, 20 Module), sw.js (v280), gema_recent. Drift-Guards nachgeführt: notify_prefs_gating (24 Gruppen, Monteur 15), rolematrix_golden regeneriert (76 Module) + 3 Layer-B-Tupel. Tests: `scripts/planablage_engine_test.mjs` (Node, 64) + `scripts/planablage_smoke_test.mjs` (Playwright, 72 — Boot/Karten/Freigaben-Roundtrip inkl. Gewerk+Layer-Sicht, Viewer mit pdf.js-Stub: Rechteck→normierte Koordinaten→Autosave→Undo, Multi-Layer-Farbwechsel, Layer-Panel/👁, Extern-Filter + Autosave-Integrität über unsichtbare Layer, Pin→Pendenz, Monteur-Gating + Statusmaschine + Notifys, Cross-Org lesen/bearbeiten/nichts, Upload-Mock inkl. 413-LIMIT, Deep-Link, Kein-Zugriff). Test-Hooks `window._pabHooks`.
