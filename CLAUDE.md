...
Lager-/Logistikmodul (Präfix `if_`, cat **Infrastruktur**): bestellte Sanitärapparate verschiedener Lieferanten importieren, den **Wareneingang kontrollieren** (was ist angekommen, was fehlt) und **Regal-Etiketten** mit der Projekt-Adresse drucken. Zielperson im Alltag ist der Projektleiter — der Ablauf muss schnell und fehlerarm sein. Umsetzung nach dem Wareneingang-Handoff, **an die GEMA-Architektur angepasst** (per-Record-Sync statt eigener Supabase-Tabellen; `GemaObjekte` als Projekt/Adresse-Entität statt `we_projekt`; `GemaProdukte`-Lieferanten statt zweiter Lieferantentabelle).

- **Storage: per-Record in der Cloud** (moduleKey `wareneingang`): Lieferungen `we:` → `gema_we_pool_v1` (**Positionen eingebettet** als Array — eine Lieferung = ein Record; kein zweiter Pool/Join), Lieferanten-Spalten-Mappings `wemap:` → `gema_we_map_pool_v1`. Einzel-Saves via `GemaSync.saveRecord` (NIE `persistCollection` — Pool global über alle Orgs, wie GemaBest/Werkzeug-Dashboard), `bindCollection` beim Boot, Org-Scoping über `l.orgId`. Einstellungen in `org.settings.wareneingang` (`{labelW,labelH,showArtikel,numberLabels,standardObjektId,lager:{name,strasse,plz,ort}}`).
- **Lieferung-Record**: `{id,orgId,erstelltVon/Name,importDatum,lieferantId,lieferantFirma,bestellnummer,bestelldatum,notiz,positionen:[{id,sortindex,artikelNr,bezeichnung,menge,eingegangenMenge,status,projekt:{objektId,name,strasse,plz,ort}}],status,updatedAt}`. Status berechnet: Position `offen`→`teilweise`→`eingegangen` (aus menge/eingegangenMenge), Lieferung `offen`→`teilweise`→`komplett`.
- **4 Tabs**: **Übersicht** (Sammelpool mit KPI-Filter-Chips, Suche, Lieferant-/Projekt-/Sortier-Filter; Detail-Modal mit **Wareneingangsmodus**: pro Position eingegangene Menge / «＋1» / «✓ voll», Teilmengen/Backorder, «✓ Alles eingegangen», Etiketten-Nachdruck). **Import** (3-Schritt-Wizard). **Schnell-Etikette** (manuelle Einzel-Etikette mit Live-Vorschau + optionaler Projekt-Übernahme). **Einstellungen** (Etiketten-Format, Standardprojekt/Lager, Mapping-Liste).
- **Import-Wizard**: (1) Quelle & Lieferant — Lieferant aus `GemaProdukte.getAllLieferanten()` ODER Freitext («anderer Lieferant»); Quelle «Tabelle einfügen» oder «PDF». (2) Extraktion — **HTML-Einfügen** fängt das `paste`-Event ab (`clipboardData.getData('text/html')` → `DOMParser` → Kandidaten-`<table>` mit den meisten Datenzeilen; Fallback text/plain mit Tab/2-Space-Split); **PDF** via **pdf.js** (lazy von cdnjs, `window.pdfjsLib`), Textitems mit x/y → Zeilen nach y clustern → Spalten nach x-Lücken clustern. **Mapping-Assistent** bei fehlendem Mapping: extrahiertes Raster + Spalten-Zuordnung (Art-Nr/Bezeichnung/Menge/Kopfzeilen-Skip) mit Heuristik-Vorschlag (`guessCols`: Header-Keywords, längste Textspalte, Ganzzahl-Spalte), gespeichert pro Lieferant+Quelle (`saveMapping`). Beim nächsten Import automatisch. **PDF speichert x-Bänder** (`colsToBands` → `applyPdfBands`, stabiler als Spaltenindizes), HTML Spaltenindizes (`applyGridMapping`). (3) **Review-Grid** (immer editierbar, Pflicht-Kontrolle weil PDF fehleranfällig): `Menge | Art-Nr | Bezeichnung | Projekt`, Zeilen editierbar/löschbar/manuell ergänzbar, Menge 0 rot markiert; **«Projekt für ganze Lieferung»** (Autocomplete aus GemaObjekte) → «⤓ auf alle Zeilen»; Standard «Lager / kein Projekt» vorausgewählt; pro Zeile überschreibbar (Split-Lieferungen); **neues Projekt inline** (`GemaObjekte.upsertObjekt`, ADD-ONLY, mit `GemaAdresse`-Autocomplete); Bestell-Nr/Datum + Duplikat-Warnung (gleiche Bestell-Nr + Lieferant); «Importieren» bzw. «Importieren & Etiketten drucken».
...
- Test-Hooks: `window._weHooks` (`settings/liefStatus/posStatus/extractHtmlGrid/extractTextGrid/guessCols/applyGridMapping/pdfItemsToGrid/applyPdfBands/mkPos`). Playwright-Smoke (localStorage-Seeding Lagerist vs. Monteur, externe Hosts geblockt): 22 Checks (Zugriff, 4 Tabs, Etiketten-Auto-Fit, HTML-/Text-Grid-Extraktion + Spalten-Heuristik, «Kein Zugriff» für Monteur).
- Registriert: gema_auth (MODULES `wareneingang` cat Infrastruktur, FILE_MAP `if_wareneingang`, **neue Rolle `role_lagerist`** + Migration `gema_auth_lagerist_v1`, KATEGORIE_ROLLEN in allen Gebäudetechnik-Kategorien), index.html (Infrastruktur-Kachel `data-module="wareneingang"`), sw.js (v215), gema_recent (PAGE_LABELS). Kein GemaNotify-Event in v1 (rein org-intern).
