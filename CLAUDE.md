...
**iPhone-Ansicht in den Modulen (gema_native_mobil.js, 07/2026 — die 6 vorbereiteten Screens sind LIVE verdrahtet):** `GemaNativeMobil.mount({render})` hängt einen `.gn.gn--page`-Vollbild-Screen (fixed, z-index 900 — unter allen Modals ≥10500) in die Modulseite: aktiv NUR ≤740px (`phone()`), Desktop/Tablet unverändert; `html.gn-native-on` versteckt `.g-nav` + sperrt Body-Scroll; Umschalter **«Klassische Ansicht»** (`[data-gn-classic]`-Button im Screen, persistiert in `gema_native_view_v1`, URL-Override `?native=0/1`) + **📱-Rückkehr-Pill** unten links im klassischen Phone-Modus; Re-Render erhält die Scroll-Position; `esc()` = Voll-Escaper. Verdrahtet in 6 Modulen (Muster: Integrations-Block am Dateiende, ECHTE Daten + bestehende Modul-Aktionen — die klassischen Modals öffnen ÜBER dem Screen):
- **if_werkzeug**: Liste aus `tools` (Badges Verfügbar/Ausgeliehen/Defekt/Verloren/Prüfung via `worstDays`), Suche+Segmente, Zeile → `openViewTool`, Long-Press-Ctx (Öffnen/QR/Defekt), 📷 → `_wzScanWithCamera`, ＋ → `openAdd` (nur `_wzCanEdit`). Refresh: `window.renderList` per Zuweisung gewrappt (Hoisting-Regel b).
- **if_fahrzeug**: Daten via `window._fzPermHooks.vehicles()` (Modul ist IIFE-gekapselt), Fahrer-Chip (`gn-ava-sm`), MFK-fällig-Badge (≤30 Tage), Aktionen `openViewFzg`/`_fzQuickKmEdit`/`openDefekt`/`openAddVehicle` (alle window-exponiert). Refresh: 3-s-Signatur-Poll (Modul-`render()` nicht wrapbar).
- **pm_stunden**: Wochen-Summe + Tages-Gruppen aus `stTagFor`/`stdEintragMin`, Vorwoche/Heute/Nächste-Seg (steuert `_wkStart`), Karte mit ＋ Zeit erfassen/Absenz/Woche einreichen → bestehende Dialoge (`stEinNeu`/`stAbsOpen`/`stWocheEinreichen`), Eintrag-Tap → `stEinEdit`. Refresh: `window.stRender` gewrappt.
- **pm_einsatzplan**: Wochenstreifen (`gn-day` mit Event-Dots) + Agenda aus `epAll`/`epCovers`; Monteure sehen NUR eigene Einsätze inkl. `bsChipsHtml`+`epSchluesselBoxHtml`, Planende (epCanPlan) alle + Tap → `epEvOpen` + ＋ → `epNeu`; Event-Farbe = Arbeitsbereich (`abById`). Refresh: `window.epRender` gewrappt.
- **sys_workspace**: Cockpit mit ECHTEN KPIs direkt aus den Pool-Caches (Stunden Woche eigene, Einsätze heute, Werkzeug-Prüfungen ≤30 T., offene Regie-Rapporte — kleine Fälligkeits-/Minuten-Helfer bewusst lokal dupliziert), «Zuletzt verwendet» via `GemaRecent.list()` (aktuelle Seite gefiltert), Eimer-Liste (Tap → klassische Ansicht).
- **sb_druckdispositiv (DOM-Proxy-Muster für Berechnungsmodule)**: Native-Felder spiegeln die klassischen Inputs (Wert setzen → `recalc()`), Modus-Seg ruft `setMode('hoehen'|'direkt')`, das Ergebnis liest `#out-*`-Werte + Normstatus (`#rs-fliessdruck`, Kartenfarbe aus `rc-fliessdruck`-Klasse ok/warn/crit) — die Engine bleibt unangetastet.
- Registriert: sw.js v340 (gema-native.css/js + gema_native_mobil.js), `<link gema-native.css>` in den 6 Modul-Heads. Test: `scripts/native_screens_smoke_test.mjs` (47 Checks — Phone-Viewport 390×844 mit PostgREST-Row-Mock [WICHTIG: leerer Cloud-Mock würde die Seeds wegwischen], alle 6 Screens mit echten Daten, Filter/Toggle/Pill, Modal-über-Screen, Proxy-Berechnung, Desktop-Gegenprobe).
