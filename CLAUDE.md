...
Bestehendes SIA-118-Modul (mehrere Protokolle pro Objekt im per-Objekt-Blob `gema_abnahme_sia_v1__<objektId>` via `_GemaDB`, Mangel-/Plan-Pin-Fotos nach GemaStorage ausgelagert, 4 Unterschriften-Pads).

**Feedback-Umbau 07/2026 (Sandro, `scripts/abnahme_feedback_test.mjs` 43 Checks — deckt auch die Apparateliste ab):**
...
- **Unterschriften**: Labels «Name / Vorname» + **«Firma» statt Visum** (Storage-Key bleibt `sig.<k>.visum`) — Firma wird aus dem Kopf vorbefüllt (Unternehmer/Bauherr/Bauleitung; bewusst geleerte Felder via `_visumTouched` nie erneut befüllt); **einzeln speicherbar** (💾-Button pro Pad → persistAll).
