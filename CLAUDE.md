...
- **`format` = Ziel-Export-Format (KRITISCH, 07/2026)**: Der DataExpert-Parameter `format` wählt das **Ziel-System-Exportformat**, NICHT JSON-vs-XML als Transport. `debim` (DataExpert®BIM) ist schwer (BIM-Attribute, Bilder inline, liefert XML). **Default ist `bexio`** — und **bexio ist eine CSV** (Content-Type `text/comma-separated-values`, Semikolon-getrennt, doppelt-gequotet, deutsche Spaltenköpfe), KEIN JSON. Per Live-Diagnose verifiziert (Lieferant 1285: HTTP 200 **ohne Vertrag/Token**, nur Kopfzeile bei 0 Treffern). Pro Modus konfigurierbar: `DATASELECT_FORMAT_SUCHE` / `DATASELECT_FORMAT_BILD`, Default beide `bexio`.
- **Antwort-Parsing: JSON ODER CSV (KRITISCH)**: Der Proxy versucht zuerst `JSON.parse` (BOM/Whitespace weg); scheitert das, **parst er CSV** (`_parseCsv`: Delimiter-Sniffing `;`/`,`/`\t` aus der Kopfzeile, Quote-Handling inkl. `""`-Escape und Feldern mit eingebettetem Semikolon/Zeilenumbruch → `{header,rows}`; `null` wenn XML/HTML/JSON). **Nur-Kopfzeile = 0 Treffer** (leere Artikelliste, kein Fehler). Erst wenn beides scheitert (echtes XML/HTML) kommt die «kein verwertbares Format»-Meldung. `_normArtikel` mappt die bexio-CSV-Spalten mit: `Produktcode`→artnr, `Produktname`/`Produktbeschreibung`→bezeichnung (HTML via `_stripHtml` entfernt), **`Verkaufspreis`→preis (Einkaufspreis nur als letzter Ausweg!)**, `Einheit`, `Währung`, `Hauptgruppe/Untergruppe`→serie. bexio-CSV hat **keine EAN-Spalte** — die Suche schickt 12–14-stellige Zahlen trotzdem als `EAN`-Parameter (`_erpDsClassify`: EAN vs. Artikel-Nr vs. Bezeichnung).
