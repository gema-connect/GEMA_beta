...
- **KI-Analyse (`netlify/functions/claude-plan.js`, Redirect `/api/claude-plan`)**: erzwungenes Tool-Use, **zwei Modi** — `grundriss` (Pass 1: plantyp, geschoss/-index, massstab, `bemassungen:[{wert_mm,p1,p2}]` normalisiert 0..1, `raeume:[{nummer,name,angeschriebene_flaeche_m2,label_position,typ,nutzung,konfidenz}]`) und `schnitt` (Pass 3: `geschosse:[{bezeichnung,geschoss_index,lichte_hoehe_m,konstruktionsstaerke_m}]`, dachneigung_grad, dachform, kniestock_m). Modell Default `claude-sonnet-5` (Plan-Lesen ist anspruchsvolle Vision; wenige, gecachte Aufrufe), Env `ANTHROPIC_PLAN_MODEL`. Client `GemaClaude.analyzePlan({imageBase64,mediaType,text,modus})` — Canvas wird auf 1568 px skaliert (JPEG 0.85), PDF-Textlayer als Kontext mitgegeben. `plApplyGrundriss` legt NUR Seeds an (Duplikat-Schutz über Raumnummer/Name), Kalibriervorschlag aus Bemassungen; `plApplySchnitt` schreibt Geschosshöhen ins Projekt (Match über bezeichnung/geschoss_index). **Ergebnis idempotent gecacht** (`s.analyse`) — «Neu analysieren» nur nach Confirm; «🪄 Polygone aus Seeds» füllt alle offenen Seeds. Ohne Function/Key: klare Meldung, Modul läuft manuell weiter.
- **Auswertung (nur freigegebene Flächen)**: Σ beheizt/unbeheizt/Aussenräume netto · BGF aus Kategorie `kontur` (Aussenkontur pro Geschoss) · **EBF** = BGF beheizter Geschosse, Fallback Netto × `ebfFaktor` (Default 1.2, klar als ⚠ Annahme markiert) · **Hülle**: Fassade = Σ Kontur-Umfang × Geschosshöhe (lichte + Konstruktion, aus Schnitt-Analyse oder manueller Tabelle — fehlende Höhen werden benannt), Dach = BGF oberstes Geschoss / cos(Neigung), Boden = BGF unterstes Geschoss, **Dämmfläche** = Fassade + Dach + Boden − Fenster − Türen (Fenster/Türen manuell, ⚠). Export CSV (Excel, Semikolon+BOM) + JSON via überschreibbarem `plDownload`.
- Rechte: Planer/Admin/Abteilungsleiter via `_allPerms`, `role_architekt` r/w (`plaene`). Registriert: gema_auth (MODULES `plaene` cat Projektmanagement, FILE_MAP `pm_plaene`), index.html (PM, 19 Module), sw.js (v241), gema_recent, netlify.toml. KEIN GemaNotify-Event (org-intern). Boot-Guard: `plPrjSel` fehlt (Kein-Zugriff-Body) → Init bricht ab. Tests: Node-Engine 32 + Playwright-Smoke 35 (Seeding, Summen/EBF/Hülle exakt, Freigabe, Sanity-Ampel, KI-Apply gemockt, CSV, Zugriff Admin/Architekt/Monteur, index-Kachel) + Fill-E2E 7 (synthetischer Grundriss auf Canvas: Closing schliesst 18-px-Tür, Fläche px-genau, Türleck → 60-%-Guard).
