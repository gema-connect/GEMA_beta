...
- **Mitarbeiter-Stammdaten** in `org.settings.stunden.mitarbeiter[userId] = {eintritt,austritt,ferienTage?}` (⚙️-Modal, Tabelle aller Org-User) — Basis für den Pro-rata-Ferienanspruch; leer = ganzes Jahr / Standard-Anspruch.
...
- Registriert: gema_auth (MODULES `stundenerfassung` cat Projektmanagement, FILE_MAP `pm_stunden`, Monteur/Spengler/Magaziner rw, Planer via `_allPerms`), gema_notify (`stunden_eingereicht`/`stunden_entscheid`/`stunden_topfb`/`stunden_auszahlung`), index.html (PM, 15 Module), sw.js.
