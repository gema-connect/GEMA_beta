...
- **🔎 DataSelect**: IGH-Lieferantenkatalog (siehe Abschnitt «DataSelect-Lieferantenkatalog»), Artikel inkl. Bild
...
**Zeilen markieren / verschieben / einfügen (07/2026, `scripts/erp_positionen_test.mjs` 25 Checks)**: Jede Positionszeile hat im editierbaren Zustand einen **⠿-Griff** (`cells[0]` — Tests, die per Zellindex lesen, +1 beachten). **Auswahl** (`_selIds`/`_selAnchor`, `erpPosSelClick`): Klick = einzeln (erneuter Klick hebt auf), **Shift** = Bereich vom Anker, **Ctrl/Cmd** = einzelne umschalten; markierte Zeilen tragen `.pos-sel`. **Delete-Taste** (`document`-keydown, nur bei offenem+editierbarem Editor, Auswahl vorhanden und Fokus NICHT in Input/Select/Textarea) löscht die Auswahl (`erpPosDeleteSelected`). **Drag&Drop** über den Griff (`erpPosDragStart/Over/Drop`, id-basiert, vor die Zielzeile; Touch nutzt das Kontextmenü ⬆️⬇️). **Einfügen respektiert die Markierung** (`_erpInsertPos`): bei GENAU einer markierten Zeile wird die neue Position/der Katalog-/DataSelect-Artikel DIREKT darüber eingefügt, bei mehreren (oder keiner) ans Ende — genutzt von `erpPosAdd`, `erpPickTake`, `erpDsTake`, `erpEigInsert` (BKP-Massen-Insert hängt bewusst an). Positions-Kontextmenü zusätzlich «Neue Position darüber/darunter» + «N markierte löschen». Auswahl wird bei jedem vollen Editor-Rebuild (`erpOpenEditor`) und beim Öffnen/Schliessen zurückgesetzt.

**Sperren/Entsperren (07/2026)**: `erpNaturalEditable(d)` = natürlich editierbar (Offerte nur Entwurf — **versendet ist gesperrt**; Auftrag solange nicht abgeschlossen; Rechnung nur Entwurf). `erpEditable()` = natürlich editierbar ODER **transient entsperrt** (`cur._unlocked`); `erpLocked()` = berechtigt aber gesperrt und nicht entsperrt → Banner + «🔓 Entsperren»-Button (`erpEntsperren`) im Editor-Kopf und -Footer. `_unlocked` wird NIE gespeichert (in `erpSaveCur` gestrippt) — beim erneuten Öffnen ist das Dokument wieder gesperrt.
...
**BKP-Titel (07/2026, `scripts/erp_bkp_titel_test.mjs` 23 Checks)**: Titelzeilen tragen optional `bkp` (Nummer) und werden **nach Ebene eingerückt wie die BKP-Checkliste im Ausschreibungsmodul** (`bkpEbene`: 1-stellig=0 · 2-stellig=1 · 3-stellig=2 · mit Punkt (254.0)=3; Nr-Feld + Text-Feld in der Titelzeile — beide frei anpassbar, die Einrückung folgt der Nummer). Der **«🧱 BKP-Titel»-Picker** (`erpBkpOpen`, Modal mit Suche + eingerückter Liste aus `gema_bkp_katalog.js` — kompletter Standard-BKP, 349 Einträge) fügt gewählte Nummern in Baumreihenfolge als Titelzeilen ein (lexikografische Sortierung der IDs = BKP-Reihenfolge). PDF: BKP-Nummer statt Kapitel-Buchstabe als `grpltr` (eingerückt); **Zusammenfassung mit Teilbaum-Rollup** — Ober-Titel zeigen die Summe ihres Teilbaums (bkp-Präfix-Match), Buchstaben-Kapitel ohne bkp bleiben Direktsummen (Buchstaben-Index zählt nur bkp-lose Gruppen), das Gesamttotal rechnet unabhängig über die Positionen (kein Doppelzählen). Vorlagen behalten `bkp`.
...
Artikel eines IGH-Lieferanten aus **dataselect.ch** (DataExpert®BIM) direkt in eine Offerte/Rechnung einfügen — mit Bezeichnung, Preis, Einheit, EAN und **Produktbild** (07/2026, Node-Test `scripts/dataselect_norm_test.mjs` 33, Playwright `scripts/dataselect_test.mjs` 22).
- **JWT-gated Netlify-Proxy `netlify/functions/dataselect.js`** (Redirect `/api/dataselect`, Review-S3-Muster `requireAuth`): der Browser kann dataselect.ch wegen CORS nicht direkt rufen, und Zugangsdaten bleiben serverseitig. **Fixe Ziel-Host** (`DATASELECT_BASE`, Default `https://www.dataselect.ch/api/Artikel/Get`) → keine SSRF-Fläche; Params `anbieter`(=id_anbieter, Pflicht, 1–7 Ziffern)/`artnr`/`bez`(=Bez)/`ean`(=EAN, mind. eines)/`sprache`(de/fr/it)/`preisbuch`(=preisbuch_nr) + **`format=debim` hart gesetzt** (mit Bildern). Optionaler `DATASELECT_API_KEY` (falls der Vertrag einen Token verlangt) → als `Authorization: Bearer` ODER, mit `DATASELECT_KEY_PARAM`, als Query-Param. 404 → leere Liste, Nicht-JSON/401/403 → klare Meldung («Zugang/Token prüfen»), 9s-Timeout.
- **Schema-Robustheit (KRITISCH)**: Das exakte debim-JSON-Feldschema ist vertrags-/versionsabhängig. `_normArtikel`/`_extractArray` bilden darum **heuristisch** über viele plausible Feldnamen (deutsch/englisch/BIM: ArtNr/artikelnr/articleNumber, Bezeichnung/Bez(+Bez2)/description, Bruttopreis/listPrice, Einheit/VPE/unit, Bild/Bilder[]/images[{url}]/data:-URL, Container `{Artikel:[…]}`/`{data:[…]}`/Array/Einzelobjekt) auf `{artnr,bezeichnung,ean,preis,waehrung,einheit,hersteller,serie,bildUrl}` ab; Schweizer/deutsche Preisformate (`1'234.50`/`1.234,50`/`12.30 CHF`) via `_num`. Weicht eine reale Antwort ab → Kandidatenlisten in beiden Dateien ergänzen (Node-Test deckt die Varianten ab). Exporte `_normArtikel/_extractArray/_num/_pick/_bild` fürs Node-Testing.
- **Client `gema_dataselect.js`** (`GemaDataSelect`): `search({anbieter,artnr?,bez?,ean?,sprache?,preisbuch?})` (JWT via `_authHeaders`, robustes `_parse` gegen HTML-Antworten), `anbieter()` (aus `org.settings.dataselect.anbieter`, Default-Seed **Geberit 1900** als dokumentiertes Beispiel — keine erfundenen IDs), `addAnbieter(id,name)` (org-weit via `updateOrgSettings`), `normArtikel` (Client-Fallback, Node-testbar), `fmtPreis`.
- **ERP-Integration (pm_erp)**: «🔎 DataSelect»-Button im Positions-Editor (neben 📦 Katalog) → Modal `#dsModal` mit Lieferant-Select (+ «＋» zum Hinterlegen einer IGH-ID), Suchfeldern Artnr/Bez/EAN, Sprache; Ergebnisliste mit Thumbnail/Artnr/Preis. `erpDsTake` fügt eine Position `{art:'frei', bez, menge:1, einheit, ep:preis, produktId:'ds:<artnr>', dsArtnr, lieferantFirma, bildUrl|bildDataUrl}` ein — das Bild rendert im Positions-Editor und wandert über die bestehende Positionsbild-Logik (`tr.bildrow`) ins Druck-PDF. Externe Bild-URL wird direkt gespeichert (data:-URL als `bildDataUrl`).
- Registriert: sw.js (v317, CACHE_FILES), Script-Include in pm_erp, netlify.toml (`/api/dataselect`). ENV (Netlify, alle optional): `DATASELECT_BASE`, `DATASELECT_API_KEY`, `DATASELECT_KEY_PARAM`. Ohne Deploy/Key läuft das Modul weiter (Picker meldet den Fehler, restliche Positions-Quellen unberührt).
