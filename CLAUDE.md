...
- **Token-lose Session ≠ leere Cloud (KRITISCH, Praxisfall 17.07. «es zeigt mir keine Daten mehr an»)**: Eine Session OHNE `token` (Anmeldung aus der Zeit vor Secure v1, oder Token nach einem 401 entfernt) liest mit dem anon-Key — unter aktivem RLS kommt dann **HTTP 200 + leer** zurück (kein 401, kein Redirect): alle per-Record-Pools wirken leer, und `bindCollection` hätte die gefüllten lokalen Caches mit `[]` überschrieben. Guard in gema_sync.js: `_tokenlessSession()` (userId ohne token) ⇒ eine LEERE Collection-Antwort leert einen GEFÜLLTEN Cache NIE (Cache wird zurückgegeben) und `_showRelogin()` zeigt den amber Banner «Sitzung ohne gültiges Anmelde-Token — Neu anmelden» (`#gema-sync-relogin-banner`, nie auf sys_login). Leerer Cache (frisches Gerät) verhält sich wie bisher. Writes sind ohnehin sicher (RLS lehnt ab → Outbox). Test-Harness: `seed()` in `scripts/rolematrix_harness.mjs` liefert Sessions mit Fake-JWT (`opts.tokenlos:true` für den Negativfall). Drift-Guard: `scripts/sync_tokenless_guard_test.mjs` (9 Checks).
