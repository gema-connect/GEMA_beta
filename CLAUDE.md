...
- **if_werkzeug (voll nativ)**: Liste aus `tools` (Badges via `worstDays`), Suche+Segmente; Zeile/Scan/`?view=`-Deep-Link → **natives Detail-Sheet** (`openToolSheet` — Badges, Grunddaten-`gn-kv`, Prüftermine farbig, Aktions-Liste; `window.openViewTool` ist gewrappt, Koffer + Nicht-gefunden fallen aufs klassische Detail-Modal). Native Sheets: **Erfassen/Bearbeiten** (reduzierte Felder → `openAdd()`/`editTool(id)` befüllt ALLE klassischen Felder, native übersteuern, `submitForm()` — nicht-native Felder wie Kaufbeleg/Prüfungen bleiben erhalten; Coachmark-Tour des klassischen Formulars wird via `GemaCoachmarks.markDone` stillgelegt, sonst startet sie 400 ms später überm geschlossenen Modal), **Defekt melden** (inkl. Fotos via `_wzReportPhotosChanged` — Fotos VOR `openDefektMelden(id)` stashen, das resetet `_wzReportPhotos`; dann `_wzSaveDefekt`), **Ausleihen** (`_wzOpenAusleihe`→`ausleiheUser`→`_wzSaveAusleihe`; Guard: fehlt `#ausleiheUser`, hat ein GemaDialog-Guard abgebrochen — NICHT speichern, sonst würde uid='' die Ausleihe aufheben), **Zuweisen** (Modus-Chips Hauptnutzer/Platz → `openZuweisung`+`_wzZuwTab`+`_wzSaveZuweisung`). Rückgabe/Selbst-Ausleihe/Verloren laufen über die bestehenden GemaDialog-Flows. Refresh: `window.renderList` gewrappt.
- **if_fahrzeug (voll nativ)**: Daten via `window._fzPermHooks.vehicles()` (Modul ist eine async-IIFE — NUR window-exponierte Funktionen nutzbar!), Fahrer-Chip, MFK-Badge; Zeile/`?view=` → **natives Detail-Sheet** (`openFzgSheet`, `window.openViewFzg` gewrappt; offene Defekte als Warnbox). Native Sheets: **Erfassen/Bearbeiten** (`openModal(id)`/`openAddVehicle()` → native Felder übersteuern → `saveVehicle()`; leere Nr = Auto-Nummer), **km-Stand** (echte Kette via `_fzQuickKmEdit` mit SYNCHRON gestubbtem `window.prompt/confirm` — tieferer Stand = native Zwei-Tap-Bestätigung statt confirm), **Defekt** (`openDefekt`→`defektDesc/Prio/Km`→`saveDefekt`), **Ausleihe** (`_fzOpenAusleihe`→`fzAusleiheUser`→`_fzSaveAusleihe`, Monteur self-only gespiegelt), **Fahrer zuweisen** (`openZuweisung`→`saveZuweisung`). Kosten/Garage/QR klassisch über dem Screen. Refresh: 3-s-Signatur-Poll.
...
- Registriert: sw.js v344 (gema-native.css/js + gema_native_mobil.js), `<link gema-native.css>` in den 6 Modul-Heads; die Einstellung liegt in `sys_profil.html`. Test: `scripts/native_screens_smoke_test.mjs` (79 Checks — Phone-Viewport 390×844 mit PostgREST-Row-Mock, alle 6 Screens mit echten Daten; native Roundtrips für pm_stunden/pm_einsatzplan [Zeit/Termin erfassen] UND if_werkzeug/if_fahrzeug [Detail-Sheet → Bearbeiten vorbefüllt → submitForm-Kette, Neues Gerät, Defekt-Bericht, km-Update inkl. Zwei-Tap-Bestätigung, Fahrzeug-Defekt-Event], Einstellungs-Toggle klassisch/native, Desktop-Gegenprobe).
