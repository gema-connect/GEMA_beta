...
- Registriert: sw.js v345 (gema-native.css/js + gema_native_mobil.js), `<link gema-native.css>` in den 6 Modul-Heads; die Einstellung liegt in `sys_profil.html`. Test: `scripts/native_screens_smoke_test.mjs` (100 Checks — Phone-Viewport 390×844 mit PostgREST-Row-Mock, alle 6 Screens mit echten Daten; native Roundtrips für pm_stunden/pm_einsatzplan [Zeit/Termin erfassen] UND if_werkzeug/if_fahrzeug [Detail-Sheet → Bearbeiten vorbefüllt → submitForm-Kette, Neues Gerät, Defekt-Bericht, km-Update inkl. Zwei-Tap-Bestätigung, Fahrzeug-Defekt-Event], Zurück-Taste [Injektion + Navigation zu index.html ohne Verlauf], Autocomplete in den Sheets [Bezeichnung→Kategorie-Autofill, Hersteller-Kreuzfilterung, Fahrzeug-Modell/Fahrer], Kategorie-/Typ-Chips, Filter-Sheets [13 Werkzeug-Kriterien via _wzAdvFilter + Fahrzeug-Zuteilung, Badge + Zurücksetzen], Einstellungs-Toggle klassisch/native, Desktop-Gegenprobe).
