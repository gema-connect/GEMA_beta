...
### Handelsregister-Anbindung (Zefix, gema_zefix.js)
...
- **JWT-gegateter Netlify-Proxy `netlify/functions/zefix.js`** (Redirect `/api/zefix`, `requireAuth` wie die KI-Proxies — kein offener Behörden-Proxy): `?name=<Suchbegriff>` → Trefferliste, `?uid=CHE-123.456.789` → Detail **inkl. Adresse** (die Zefix-Suche liefert bewusst keine Adresse — Muster «Suche leicht, Detail beim Auswählen» wie das Bild-Nachladen in DataSelect). **Fixer Host aus der Env → keine SSRF-Fläche**; UID wird streng validiert (`_uidDigits`), bevor sie in den Pfad geht. 9-s-Timeout.
- **Normalisierung heuristisch** (`_normFirma`/`_normAdresse`/`_extractList`/`_pick`, Node-Test `scripts/zefix_norm_test.mjs` 44 Checks): mehrsprachige Felder (`{de,fr,it}` via `_txt`), UID-Formatierung `CHE123456789 ⇄ CHE-123.456.789`, Adresse aus `street`+`houseNumber` bzw. `poBox`, Container-Varianten (Array / `{list:[…]}` / Einzelobjekt). Ergebnis: `{name,uid,uidFormatted,chid,sitz,rechtsform,rechtsformKurz,status,aktiv,zefixUrl,strasse,plz,ort}`. Weicht eine reale Antwort ab → Kandidatenlisten dort ergänzen.
- **Client `gema_zefix.js`** (`GemaZefix.attach(input,{onSelect,onClear})` · `search(name)` · `detail(uid)`): Vorschlags-Dropdown im `.gema-hr-*`-Namensraum (eigene Klassen, damit ein Firma-Feld gleichzeitig ein Adress-Autocomplete tragen kann), Debounce 320 ms, ab 3 Zeichen, Tastatur ↑/↓/Enter/Escape, gelöschte Firmen mit Badge. `onClear` feuert, sobald der Benutzer den Namen wieder tippt (programmatisches Setzen ist via `ctx.suppress` ausgenommen) — das Modul kann so einen gemerkten HR-Bezug fallen lassen.
- **Konfiguration (Netlify-Env)**: `ZEFIX_USER` + `ZEFIX_PASSWORD` (kostenloses Konto auf zefix.admin.ch → «Public REST API»), alternativ `ZEFIX_AUTH` (fertiger Basic-Header), optional `ZEFIX_BASE`. **Ohne Zugangsdaten** antwortet Zefix mit 401 → die Function meldet einen klaren Konfigurations-Hinweis, das Dropdown zeigt ihn an und **das Feld bleibt normal tippbar** (Graceful Degradation wie DataSelect ohne IGH-Vertrag). Registriert in sw.js (CACHE_FILES) und netlify.toml.
...
| `gema_zefix.js` | **Handelsregister-Anbindung (Zefix)**: `GemaZefix.attach(firmaInput,{onSelect,onClear})` hängt Vorschläge aus dem Schweizer Handelsregister an ein Firma-Feld; `search(name)`/`detail(uid)` als reine API. Abruf über den JWT-gegateten Proxy `netlify/functions/zefix.js` (`/api/zefix`). Ohne konfigurierten Zugang klarer Hinweis statt stiller Fehler — Feld bleibt tippbar. Siehe «Handelsregister-Anbindung (Zefix)» |
