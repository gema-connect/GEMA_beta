...
**Produktions-Seed mit realen Herstellerdaten**: `supabase/gema_lieferanten_seed_v1.sql` (im Supabase-SQL-Editor ausführen; Rollback daneben) legt 14 reale Lieferanten (BWT, Grünbeck, Grundfos, Wilo, KSB, Nussbaum, Geberit, GF JRG, Oventrop, Taconova, Flamco, IMI, GWF, Resideo), 27 Produkte über 10 Kategorien und 5 Armaturen-Records (kvs/ζ) an — ALLE bewusst `nicht_verifiziert` (Testbasis für den Verifizierungs-Workflow). IDs mit festen Präfixen `lief_seed_`/`prod_seed_`/`arm_seed_`; `ON CONFLICT DO NOTHING` (Re-Run überschreibt NIE — auch nicht inzwischen Verifiziertes). Quelle/Generator: `scripts/lieferanten_seed_gen.mjs` (validiert gegen die Live-KATEGORIEN-Schemata + SQL-Roundtrip; Daten dort ändern, dann neu generieren); E2E-Test `scripts/lieferanten_seed_test.mjs` (20 Checks: Kataloge, matchFn, Armaturen-Δp, Dashboard-Admin-Vorschau).
