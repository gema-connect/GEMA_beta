...
- **Notifikationen** `pruefliste_vorschlag` (an Approver: AL+Org bzw. `role_admin` bei global; Deep-Link `?tab=verwaltung`), `pruefliste_freigegeben`/`pruefliste_abgelehnt` (an Ersteller). Registriert: gema_auth (MODULES `pruefliste`, FILE_MAP), gema_notify (3 Keys), gema_notify_ui (MODUL_LABELS «📋 Prüfliste» + MODUL_ZUGRIFF `{mods:['pruefliste']}`), index.html (PM-Kachel), sw.js (v339), gema_recent, sys_workspace. Drift-Guards nachgeführt: rolematrix_golden (77 Module), notify_prefs_gating (26 Gruppen). Tests: `scripts/pruefliste_engine_test.mjs` (56) + `scripts/pruefliste_smoke_test.mjs` (48 — Boot/Tabs, Begehung+Anlage+Antwort, Ergänzen mit Bemerkung+Duplikat, Anpassungs-Vorschlag→Ziel-Update, Vorschlag→Freigabe, Inline-Bewertung + `_open`-Strip, Editor-Close-Flush, Objekt-Punkte-CRUD, Bericht-HTML, Anlagenarten, Deep-Link, Kein-Zugriff Monteur). Test-Hooks `window._prHooks`.
