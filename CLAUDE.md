...
- **Vollständigkeitskontrolle beim Scannen** (`_wzScanAusleihe` zeigt bei `_wzIsKoffer` eine Sektion `#kofCtrlSection`): Scan des Koffer-QR (`?scan=<id>`) listet ALLE `kofferInhalt`-Teile mit Checkboxen **vorausgewählt** — der Nutzer wählt ab, was er physisch NICHT sieht. `_wzKofferKontrolleSave` schreibt das Ergebnis nach `koffer.letzteKofferKontrolle = {am,von,vonId,geprueft,fehlend:[{id,name}],vollstaendig}`, loggt (`_wzActLog('pruefung',…)`), meldet Fehlteile an role_magaziner (`werkzeug_koffer_fehlteil`) und ersetzt die Sektion durch ein grün/rotes Ergebnis. Rein prüfend — ändert KEINEN Ausleih-/Zuweisungs-Status (nur Doku); die letzte Kontrolle wird beim nächsten Scan als Hinweis angezeigt.
