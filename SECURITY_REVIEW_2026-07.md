...
| **S5** persist_auth Selbst-Edit | ✅ **umgesetzt** | Feld-Whitelist (`name/profile/avatar/einstellungen/password`) im Selbst-Update; alle übrigen Felder (abo, planerPremium, lieferantId, gastZugaenge, …) werden aus dem DB-Stand übernommen. Kein Datenverlust: das Lieferanten-Self-Heal löst sich weiter über die Org-Heuristik auf. **Nachtrag 27.08.2026:** die eigenen `roleIds` sind für den **Org-Admin der eigenen Org** freigegeben (`role_admin` bleibt ihm verwehrt → 403) — er darf sie ohnehin bei allen Mitarbeitenden setzen, und das stille Verwerfen sah wie ein Speicher-Bug aus. `orgId`/`active` und die vier Eskalationsfelder bleiben in JEDEM Selbst-Update eingefroren; Drift-Guard `scripts/eigene_rollen_speichern_test.mjs`. |
