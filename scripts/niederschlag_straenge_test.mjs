...
await page.evaluate(() => {
  ...
  window.addRowUmg({});
  const rows = [...document.querySelectorAll('#tbodyDach tr:not(.fl-det), #tbodyUmg tr:not(.fl-det)')];
...
{
  await page.evaluate(() => {
    const tr = document.querySelectorAll('#tbodyDach tr:not(.fl-det)')[0];
    const sel = tr.querySelector('.strangSel');
    sel.value = '__new__';
    sel.dispatchEvent(new Event('change', { bubbles: true }));
  });
  const s1 = await page.evaluate(() => ({ n: _strHooks.strState().length, name: _strHooks.strState()[0] && _strHooks.strState()[0].name, row: document.querySelectorAll('#tbodyDach tr:not(.fl-det)')[0].dataset.strangId }));
  ...
  await page.evaluate(() => {
    [1, 2].forEach(i => {
      const sel = document.querySelectorAll('#tbodyDach tr:not(.fl-det)')[i].querySelector('.strangSel');
      sel.value = '1';
      sel.dispatchEvent(new Event('change', { bubbles: true }));
    });
  });
  ok(await page.evaluate(() => [...document.querySelectorAll('#tbodyDach tr:not(.fl-det)')].filter(tr => tr.dataset.strangId === '1').length) === 3, 'Flächen 1–3 am Strang WAR-R 1');
...
console.log('■ Zusammenstellung nach Strängen');
{
  ...
  await page.evaluate(() => {
    const sel = document.querySelectorAll('#tbodyDach tr:not(.fl-det)')[2].querySelector('.strangSel');
    sel.value = '';
    sel.dispatchEvent(new Event('change', { bubbles: true }));
  });
...
console.log('■ Umbenennen');
{
  ...
  ok(await page.evaluate(() => {
    const sel = document.querySelectorAll('#tbodyDach tr:not(.fl-det)')[0].querySelector('.strangSel');
...
console.log('■ Persistenz-Roundtrip (inkl. ssState — war vorher nicht gespeichert)');
{
  ...
  const nach = await page.evaluate(() => ({
    ...
    rowSel: document.querySelectorAll('#tbodyDach tr:not(.fl-det)')[0].querySelector('.strangSel').value,
...
console.log('■ Strang löschen');
{
  ...
  ok(await page.evaluate(() => [...document.querySelectorAll('#tbodyDach tr:not(.fl-det)')].every(tr => (tr.dataset.strangId || '') !== '1')), 'Zeilen-Zuweisungen geleert (Flächen bleiben)');
