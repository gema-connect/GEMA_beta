...
console.log('■ Layer 1 — Laufzeit-Drift-Guard (live aus der App)');
{
  ...
  ok(drift.anzahl >= 22, `EVENT_KEYS-Gruppen live extrahiert (${drift.anzahl})`);
...
// Alle 22 Laufzeit-Gruppen (EVENT_KEYS; «objekte» kommt nur in Demo-Seeds vor)
const ALLE = ['ausschreibung','werkzeug','fahrzeug','lu','schadensbericht','trocknung','produktkatalog','bestellungen','regierapport','einsatzplan','goodel','abnahme','legionellen','spuelmanager','immobilien','service','stundenerfassung','revisionsunterlagen','behoerden_formulare','abos','chat','schule'];

console.log('■ Layer 2 — Sichtbarkeits-Matrix pro Rolle');
{
  let r = await visibleGroups(['role_admin'], 'index.html');
  eqSet('Admin sieht alle 22 Gruppen', r.groups, ALLE);
  await r.ctx.close();

  r = await visibleGroups(['role_planer'], 'sys_workspace.html');
  eqSet('Planer sieht alle 22 Gruppen (Vollzugang)', r.groups, ALLE);
  await r.ctx.close();

  r = await visibleGroups(['role_monteur'], 'index.html');
  eqSet('Monteur: nur seine 13 Gruppen (kein Fahrzeug/Ausschreibung/Bestellungen …)', r.groups,
    ['werkzeug','trocknung','schadensbericht','regierapport','einsatzplan','goodel','abnahme','legionellen','spuelmanager','service','stundenerfassung','abos','chat']);
  await r.ctx.close();
...
console.log('■ Layer 2 — Panel-DOM (Monteur) + Hinweiszeile');
{
  ...
  ok(sektionen === 13, 'genau 13 Gruppen-Sektionen gerendert (' + sektionen + ')');
  await ctx.close();
}
...
console.log('■ Layer 2 — Admin-Panel unverändert (alle Gruppen, keine Hinweiszeile)');
{
  ...
  await page.waitForSelector('#gnSettingsOverlay', { timeout: 5000 });
  const sektionen = await page.$$eval('#gnPrefList > div', els => els.filter(e => e.id !== 'gnPrefHint').length);
  ok(sektionen === 22, 'Admin: alle 22 Gruppen-Sektionen (' + sektionen + ')');
