...
function _safeUrl(raw){ let u; try{ u=new URL(raw); }catch(e){ return null; } if(u.protocol!=='http:'&&u.protocol!=='https:')return null; const h=u.hostname.toLowerCase(); if(h==='localhost'||h==='::1'||h==='0.0.0.0')return null; if(/^127\./.test(h)||/^10\./.test(h)||/^192\.168\./.test(h)||/^169\.254\./.test(h))return null; if(/^172\.(1[6-9]|2[0-9]|3[0-1])\./.test(h))return null; return u.toString(); }
async function _probe(url){ const ctrl=new AbortController(); const to=setTimeout(function(){ctrl.abort();},12000); try{ const r=await fetch(url,{method:'GET',redirect:'follow',signal:ctrl.signal,headers:{'User-Agent':'GEMA-FormWatch/1.0'}}); const buf=Buffer.from(await r.arrayBuffer()); return { ok:r.ok, status:r.status, hash:crypto.createHash('sha256').update(buf).digest('hex'), etag:r.headers.get('etag')||'', lastModified:r.headers.get('last-modified')||'' }; } finally { clearTimeout(to); } }
...
exports.handler = async function () {
  ...
  try {
    ...
    for (let i=0; i<(rows||[]).length; i++){
      ...
      if (!form) continue;
      const url = _safeUrl(form.sourceUrl || '');
      if (!url || !form.sourceHash) continue; // nur URLs mit gesetzter Baseline überwachen
      checked++;
      try {
